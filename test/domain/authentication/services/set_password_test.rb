...
class Authentication::Services::SetPasswordTest < ActiveSupport::TestCase
  setup do
    ...
    ENV['PROMISE_PUBLIC_KEY_FOR_VAULT_KEY_ENCRYPTION'] = @public_key.to_s

    own_private = RbNaCl::PrivateKey.generate
    @own_public = own_private.public_key.to_s

    ENV['PROMISE_PRIVATE_KEY_FOR_VAULT_KEY_ENCRYPTION'] = own_private.to_s
  end
  ...
  test 'sets everything' do
    ...
    assert event
    assert event.data[:encrypted_vault_key]
    ...
    plaintext = box.decrypt(event.data[:encrypted_vault_key])
